---
title: "Differential Item Functioning Analysis for IES DAACS Mathematics Assessment
  (v.2.0), May 2022 - May 2023, umgc1-and-ua2 Combined Sample, Analytic Sample 2,
  n = 1570"
author: "Oxana Rosca"
date: "`r Sys.Date()`"
output:
...
# Purpose: To evaluate 174 math items on DIF across two age groups, AUS and TCAUS
# Results: 91 (good) items have no DIF on age and have a significant positive correlation with Theta scores  from 174- and 159-item models, according to logistic regression analyses with graphic representations.
...
load("C:/Users/orosc/OneDrive - University at Albany - SUNY/My DAACS/math/Math_dataClean-umgc1ua2_3.RData")
```
...
DIF_LogRegAnalysis <- function(X_tmp, G_tmp, T_tmp, title, mapping_df) {
  ...
  for (j in 1:n) {
    ...
    valid_idx <- !is.na(X_tmp[, j]) & !is.na(G_tmp)  # Exclude students with NA in G_tmp
    ...
    if (nrow(data_j) > 1) {
      ...
      # Get the difficulty level from mapping_unique_math.items_umgc1ua2
      difficulty_level <- mapping_df %>%
      ...
      if (length(data_j$T_tmp) == length(fitted_values)) {
        ...
        # Count number of students per group
        students_count <- data_j %>%
          group_by(G_tmp) %>%
          summarise(n = n()) %>%
          arrange(G_tmp)

        # Create labels including sample sizes
        group_labels <- paste0(as.character(students_count$G_tmp), " (n = ", students_count$n, ")")
...
math_Age_d24_AnSamp2_tb <- 
  Propensities_CatVar_withMeta(math.items_AnSamp2, "Age_d24")
print(math_Age_d24_AnSamp2_tb)
```

### across colleges
```{r}
# aka table(math.items_AnSamp2$Age_d24,math.items_AnSamp2$college, useNA = "always")
...
math_Age_d24_AnSamp2_college_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "Age_d24","college")
...
### 174 Items
For F-scores calculated from all 174-item unidimensional model
```{r}
X_tmp <- math.items_AnSamp2 %>% 
  select(Q001:Q174)
G_tmp <- math.items_AnSamp2$Age_d24 # Grouping variable
T_tmp <- math.items_AnSamp2$Theta_math174 # Factor scores (continuous)
mapping_df_tmp<- mapping_unique_math.items_umgc1ua2

# library("ggplot2")
# library("knitr")

# Run the function
DIF_Math174_Age_umgc1ua2_AnSamp2 <- DIF_LogRegAnalysis(X_tmp, G_tmp, T_tmp, mapping_df_tmp, title = "Math174_Age_umgc1ua2_AnSamp2")
DIF_Math174_Age_umgc1ua2_AnSamp2
```

### 159 Items
For F-scores calculated from 159-item unidimensional model
Response matrix: 15 items were removed due to poor a0-parameters: < 0.3 and > 4
(Q008,11,13,17,70,73,79,82,106,123,128,130,132,168,170).
```{r}
X_tmp <- math.items_AnSamp2 %>%
  select(Q001:Q007,Q009,Q010,Q012,Q014:Q016,Q018:Q069,Q071,Q072,Q074:Q078,Q080,
         Q081,Q083:Q105,Q107:Q122,Q124:Q127,Q129,Q131,Q133:Q167,Q169,Q171:Q174)
G_tmp <- math.items_AnSamp2$Age_d24 # Grouping variable
T_tmp <- math.items_AnSamp2$Theta_math159 # Factor scores (continuous)

# Run the function
DIF_Math159_Age_umgc1ua2_AnSamp2 <- DIF_LogRegAnalysis(X_tmp, G_tmp, T_tmp, mapping_df_tmp, title = "Math159_Age_umgc1ua2_AnSamp2")
```
...
```{r}
math_Gender_AnSamp2_tb <- Propensities_CatVar_withMeta(math.items_AnSamp2, "gender")
math_Gender_AnSamp2_tb
```

### across colleges
```{r}
math_Gender_AnSamp2_college_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "gender","college")
...
### across age-groups
```{r}
math_Gender_AnSamp2_age_d24_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "gender","Age_d24")
math_Gender_AnSamp2_age_d24_tb
```
...
G_tmp <- math.items_AnSamp2$gender 

# Run the function
DIF_Math174_Gender_umgc1ua2_AnSamp2 <- DIF_LogRegAnalysis(X_tmp, G_tmp, T_tmp, mapping_df_tmp, title = "Math174_Gender_umgc1ua2_AnSamp2")
...
```{r}
math_Transfer_AnSamp2_tb <- Propensities_CatVar_withMeta(math.items_AnSamp2, "transfer")
math_Transfer_AnSamp2_tb
```

### across colleges
```{r}
math_Transfer_AnSamp2_college_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "transfer","college")
...
### across age-groups
```{r}
math_Transfer_AnSamp2_age_d24_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "transfer","Age_d24")
math_Transfer_AnSamp2_age_d24_tb
```
...
G_tmp <- math.items_AnSamp2$transfer 

# Run the function
DIF_Math174_Transfer_umgc1ua2_AnSamp2 <- DIF_LogRegAnalysis(X_tmp, G_tmp, T_tmp, mapping_df_tmp, title = "Math174_Transfer_umgc1ua2_AnSamp2")
...
## Distribution within AnSamp2
```{r}
math_Military_AnSamp2_tb <- Propensities_CatVar_withMeta(math.items_AnSamp2, "Military")
math_Military_AnSamp2_tb
```

### across colleges
```{r}
math_Military_AnSamp2_college_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "Military","college")
...
### across age-groups
```{r}
math_Military_AnSamp2_age_d24_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "Military","Age_d24")
math_Military_AnSamp2_age_d24_tb
```
...
```{r}
# Call the DIF function
G_tmp <- math.items_AnSamp2$Military 

# Run the function
DIF_Math174_Military_umgc1ua2_AnSamp2 <- DIF_LogRegAnalysis(X_tmp, G_tmp, T_tmp, mapping_df_tmp, title = "Math174_Military_umgc1ua2_AnSamp2")
...
## Distribution within AnSamp2
```{r}
math_LowSES_AnSamp2_tb <- Propensities_CatVar_withMeta(math.items_AnSamp2, "Pell")
math_LowSES_AnSamp2_tb
```

### across colleges
```{r}
math_LowSES_AnSamp2_college_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "Pell","college")
...
### across age-groups
```{r}
math_LowSES_AnSamp2_age_d24_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "Pell","Age_d24")
math_LowSES_AnSamp2_age_d24_tb
```
...
```{r}
# Call the DIF function
G_tmp <- math.items_AnSamp2$Pell 

# Run the function
DIF_Math174_lowerSES_umgc1ua2_AnSamp2 <- DIF_LogRegAnalysis(X_tmp, G_tmp, T_tmp, mapping_df_tmp, title = "Math174_lowerSES_umgc1ua2_AnSamp2")
...
## Distribution, AnSamp2
```{r}
math_Ethnicity_AnSamp2_tb <- Propensities_CatVar_withMeta(math.items_AnSamp2, "ethnicity")
math_Ethnicity_AnSamp2_tb
```

### across colleges
```{r}
math_Ethnicity_AnSamp2_college_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "ethnicity","college")
...
### across age-groups
```{r}
math_Ethnicity_AnSamp2_age_d24_tb <- 
  Propensities_TwoVars(math.items_AnSamp2, "ethnicity","Age_d24")
math_Ethnicity_AnSamp2_age_d24_tb
```

# Mean Theta159 Scores in Demographic Groups
...
y_limits_tmp <- range(math.items_samp22D$Theta_math159, na.rm = TRUE)

# Dynamically determine sample size for the title
sample_size_tmp <- nrow(math.items_samp22D)
title_tmp <- paste("Math Theta159 scores Across Demographic Variables, n =", sample_size_tmp)
...
mathTheta159_ethnicity_boxplots <- lapply(names(ethnicity_groups_tmp), function(group) {
  ggplot(ethnicity_groups_tmp[[group]], aes(x = factor(1), y = Theta_math159)) +
    ...
    labs(title_tmp = group, x = ifelse(group == "NativeAm", "Ethnicity", ""), y = ifelse(group == "All", "Math Theta159 Scores", "")) +
    theme_minimal() +
...
demographic_boxplots <- list(
  ggboxplot(math.items_samp22D, x = "college", y = "Theta_math159", color = "college",
            palette = c("black", "#999999"), ylab = "Math Theta159 Score", xlab = "College", width = 0.5) +
    stat_summary(fun = median, geom = "text",
                 aes(label = round(after_stat(y), 2)),
  ...
  ggboxplot(math.items_samp22D, x = "Age_d24", y = "Theta_math159", color = "Age_d24",
  ...
  ggboxplot(na.omit(math.items_samp22D[c("gender", "Theta_math159")]), x = "gender", y = "Theta_math159", color = "gender",
...
ggboxplot(na.omit(math.items_samp22D[c("Military", "Theta_math159")]), x = "Military", y = "Theta_math159", color = "Military",
  ...
  ggboxplot(math.items_samp22D, x = "Pell", y = "Theta_math159", color = "Pell",
  ...
  ggboxplot(math.items_samp22D, x = "transfer", y = "Theta_math159", color = "transfer",
...
pdf("Math_Theta159_Demographics_Boxplots_samp22D.pdf", width = 14, height = 8)

grid.arrange(
  arrangeGrob(
    arrangeGrob(grobs = mathTheta159_ethnicity_boxplots, ncol = 10),
...
```{r}
# Define common y-axis limits for consistency
y_limits_tmp <- range(math.items_AnSamp2$Theta_math159, na.rm = TRUE)

# Dynamically determine sample size for the title
sample_size_tmp <- nrow(math.items_AnSamp2)
title_tmp <- paste("Math Theta159 scores Across Demographic Variables, n =", sample_size_tmp)
...
# Create Demographic Boxplots (6 groups, in a single row with increased spacing)
demographic_boxplots <- list(
  ggboxplot(math.items_AnSamp2, x = "college", y = "Theta_math159", color = "college",
            palette = c("black", "#999999"), ylab = "Math Theta159 Score", xlab = "College", width = 0.5) +
    stat_summary(fun = median, geom = "text",
                 aes(label = round(after_stat(y), 2)),
                 position = position_nudge(y = 0.11),
  ...
  ggboxplot(math.items_AnSamp2, x = "Age_d24", y = "Theta_math159", color = "Age_d24",
            palette = c("black", "#999999"), ylab = "", xlab = "Age Group", width = 0.5) +
    stat_summary(fun = median, geom = "text",
                 aes(label = round(after_stat(y), 2)),
                 position = position_nudge(y = 0.11),
  ...
  ggboxplot(na.omit(math.items_AnSamp2[c("gender", "Theta_math159")]), x = "gender", y = "Theta_math159", color = "gender",
          palette = c("black", "#999999"), ylab = "", xlab = "Gender", width = 0.5) +
  stat_summary(fun = median, geom = "text",
               aes(label = round(after_stat(y), 2)),
               position = position_nudge(y = 0.11),
...
ggboxplot(na.omit(math.items_AnSamp2[c("Military", "Theta_math159")]), x = "Military", y = "Theta_math159", color = "Military",
          palette = c("black", "#999999"), ylab = "", xlab = "Military Status", width = 0.5) +
  stat_summary(fun = median, geom = "text",
               aes(label = round(after_stat(y), 2)),
               position = position_nudge(y = 0.11),
  ...
  ggboxplot(math.items_AnSamp2, x = "Pell", y = "Theta_math159", color = "Pell",
            palette = c("black", "#999999"), ylab = "", xlab = "Financial Support", width = 0.5) +
    stat_summary(fun = median, geom = "text",
                 aes(label = round(after_stat(y), 2)),
                 position = position_nudge(y = 0.11),
  ...
  ggboxplot(math.items_AnSamp2, x = "transfer", y = "Theta_math159", color = "transfer",
            palette = c("black", "#999999"), ylab = "", xlab = "Transfer Student", width = 0.5) +
    stat_summary(fun = median, geom = "text",
                 aes(label = round(after_stat(y), 2)),
                 position = position_nudge(y = 0.11),
...
# Save to PDF with improved spacing for the legend
pdf("Math_Theta159_Demographics_Boxplots_AnSamp2.pdf", width = 14, height = 8)
...
```{r}
title_tmp <- paste("Math Theta159 Scores Across Ethnicity Groups, n =", sample_size_tmp)
...
```{r}
title_tmp <- paste("Math Theta159 Scores Across Demographic Groups, n =", sample_size_tmp)

grid.arrange(
  arrangeGrob(grobs = demographic_boxplots, ncol = 3),  # Auto layout
  top = textGrob(title_tmp, gp = gpar(fontsize = 14, fontface = "bold"))
)
```
...
tibble_list_demographicsAnSamp2 <- list(
  math_Gender_AnSamp2_tb = math_Gender_AnSamp2_tb,
  math_Gender_AnSamp2_college_tb = math_Gender_AnSamp2_college_tb,
  math_Gender_AnSamp2_age_d24_tb = math_Gender_AnSamp2_age_d24_tb,
  math_Transfer_AnSamp2_tb  = math_Transfer_AnSamp2_tb,
  math_Transfer_AnSamp2_college_tb = math_Transfer_AnSamp2_college_tb,
  math_Transfer_AnSamp2_age_d24_tb = math_Transfer_AnSamp2_age_d24_tb,
  math_Military_AnSamp2_tb = math_Military_AnSamp2_tb,
  math_Military_AnSamp2_college_tb = math_Military_AnSamp2_college_tb,
  math_Military_AnSamp2_age_d24_tb  = math_Military_AnSamp2_age_d24_tb,
  math_LowSES_AnSamp2_tb = math_LowSES_AnSamp2_tb,
  math_LowSES_AnSamp2_college_tb = math_LowSES_AnSamp2_college_tb,
  math_LowSES_AnSamp2_age_d24_tb = math_LowSES_AnSamp2_age_d24_tb,
  math_Age_d24_AnSamp2_tb = math_Age_d24_AnSamp2_tb,
  math_Age_d24_AnSamp2_college_tb = math_Age_d24_AnSamp2_college_tb
)

generate_pdf_Frequencies_woTitles (tibble_list_demographicsAnSamp2, 
      "DAACS Math, Demographics, UMGC1 and UA2 Analytic Sample 2.pdf", 
                  tables_per_page = 9, title = 
      "DAACS Math, Demographics, UMGC1 and UA2, Analytic Sample 2, n = 1570")

```  

# Save all data in one file
```{r}
#save.image("D:/Dropbox/DAACS-Validity/Analyses/Math/Math_dataClean-umgc1ua2_4.RData")
save.image("C:/Users/orosc/OneDrive - University at Albany - SUNY/My DAACS/math/Math_dataClean-umgc1ua2_4.RData")
```
...
more likely to happen in datasets with sparse data, such as DAACS math scores 
