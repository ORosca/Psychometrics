---
title: "Testlet Response Theory Analysis for IES DAACS Reading Assessment (180 Items), May 2022 - May 2023, uni1-and-uni2 Combined Sample, AnSamp1, n = 4523"
author: "Oxana Rosca"
date: "`r Sys.Date()`"
output:
  html_document:
  ...
  word_document:
    toc: true
    toc_depth: 6
    reference_docx: "C:/Users/orosc/XXXXXXX/My DAACS/WordDocMarkdownTemplate.docx"
---

# Purpose: 
Item Response Theory (IRT) Analysis of DAACS Reading Items Using Bifactor Model. 
IES project, 2022-23, Uni1 and Uni2 Scores, All Items, First Attempt, Non-speedy sts 
(Analytic Sample 1).
...
# Participants
A total of 5447 participants completed at least one DAACS assessment, including 4152 (76%) from Uni1 and 1295 (24%) from Uni2. All participants had both a DAACS-assigned ID (DAACS_ID) and an institution-assigned ID.
For the reading assessment specifically, 4626 respondents participated: 3894 (84%) from Uni1 and 732 (16%) from Uni2.

## Analytic Sample 1
  Purpose: For IRT analyses.
  Composition: The Analytic Sample 1 (AnSamp1) included 4523 non-speedy respondents: 3798 (84%) from Uni1 and 725 (16%) from Uni2. 
...
A detailed dataframe "read.items_AnSamp1" includes 200 columns:180 item scores, 2 ID variables,	18 personal variables, such as reading total scores, dichotomous variables (e.g., gender, age group [below or above 24 years], and college [Uni1 or Uni2]).  
...
```{r}
load("C:/Users/orosc/XXXXXXX/My DAACS/read/read_dataClean-Uni1Uni2_1.RData")
...
(Uni1 or Uni2), etc., are present in the following dataframe. 
```{r}
# A long self-explanatory title
dim(read.items1stAtt_wide_nsp_uni1uni2_wPersonal) # 4523  200
# A short title
...
###  College: Nuni1 = 3798 (83.97%), Nuni2 = 725 (16.03%) 
```{r} 
...
# Create a loop function for PA
	...
	# Remove rows with missing values
	 ...
	 current_testlet_tmp[rowSums(is.na(current_testlet_tmp)) != ncol(current_testlet_tmp), ]
	# Perform PA for the current testlet
	 read.items_uni1uni2_parallel_analysis_plot <-
	 	fa.parallel(current_testlet_tmp, fm = "ml", fa = "fa", n.iter = 1000, SMC = F,
		main = paste("Testlet Parallel Analysis, uni1uni2, items Q", start_col_tmp,
					 "-Q", end_col_tmp))
	# Display the PA plot
	 print(read.items_uni1uni2_parallel_analysis_plot)}
```
...
read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix <- 
...
for(i in 1:ncol(read.itemsONLY_AnSamp1[,c(readTestlet_1stItem_vector_num)])){
  for(j in 1:ncol(read.itemsONLY_AnSamp1[,c(readTestlet_1stItem_vector_num)])){
    ...
    read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix[i, j] <- 
...
```{r}
min(read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix) # 17
max(read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix) # 500
```

```{r}
# Convert the matrix to a data frame with row and column names
read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix <- 
  as.data.frame(read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix)
rownames(read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix) <- 
  colnames(read.itemsONLY_AnSamp1[, c(readTestlet_1stItem_vector_num)])
colnames(read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix) <- 
  colnames(read.itemsONLY_AnSamp1[, c(readTestlet_1stItem_vector_num)])
read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix
write.csv(read.items_AnSamp1uni1uni2_ItemStudentOverlap_matrix,
          "read.items_AnSamp1uni1uni2_TestletStudentOverlap_matrix.csv")
```
...
```{r} 
# readBifactor1PL_172<-bfactor(read.items_172_uni1uni2,readTestletModelVector_172,
#                              itemtype = 'Rasch', TOL = 1e-03,
#                              SE = TRUE,
#                              accelerate = 'none',
#                              technical = list(NCYCLES = 2000))
...
```{r} 
# readBifactor2PL_172<-bfactor(read.items_172_uni1uni2,readTestletModelVector_172,
#                              itemtype = '2PL', TOL = 1e-03,
#                              SE = TRUE,
#                              accelerate = 'none',
#                              technical = list(NCYCLES = 2000))
...
The items are organized into testlets by being sorted in ascending order:
```{r}
names(read.items_173_uni1uni2) 
# Check if column names are in increasing order
all(diff(as.numeric(gsub("Q", "", names(read.items_173_uni1uni2)))) > 0) # TRUE
```
...
```{r}
readBifactor1PL_173<-
  bfactor(read.items_173_uni1uni2,readTestletModelVector_173,
                             itemtype = 'Rasch', TOL = 1e-03,
                             SE = TRUE,
                             accelerate = 'none',
                             technical = list(NCYCLES = 2000))
...
read.items_AnSamp2<- read.items_AnSamp2[,c(1, 3:20, 202, 2, 21:201)]
```
...
readBifactor2PL_uni1uni2_all_Thetas <-
  merge(Theta_read173[, c(1,32,63)],Theta_read180[, c(1,32,63)],all = T)
# Columns' names
names(readBifactor2PL_uni1uni2_all_Thetas)
```
...
densityPlot_2corMeasures(
  ...
  footnote = 
  "Note: Data collected in 2022-23 a.y. from Uni1 and Uni2 non-speedy students",
  output_file = "ReadingThetaScores_DensityPlot.pdf"
)
```
...
```{r}
#library(car) 
readBifactor2PL_uni1uni2_Theta180_qqPlot<-
  qqPlot(readBifactor2PL_uni1uni2_all_Thetas$Theta_read180)
readBifactor2PL_uni1uni2_Theta180_qqPlot
```

```{r}
readBifactor2PL_uni1uni2_Thetas173_qqPlot<-
  qqPlot(readBifactor2PL_uni1uni2_all_Thetas$Theta_read173)
readBifactor2PL_uni1uni2_Thetas173_qqPlot
```
...
Theta_data_tmp <- data.frame(
  Theta = c(readBifactor2PL_uni1uni2_all_Thetas$Theta_read180_AnSamp1, readBifactor2PL_uni1uni2_all_Thetas$Theta_read173_AnSamp1),
  Group = rep(c("Theta_read180", "Theta_read173"), 
              times = c(length(readBifactor2PL_uni1uni2_all_Thetas$Theta_read180_AnSamp1), length(readBifactor2PL_uni1uni2_all_Thetas$Theta_read173_AnSamp1)))
)
...
```{r} 
#save.image("D:/Dropbox/DAACS-Validity/Analyses/Reading/read_dataClean-Uni1Uni2_2.RData")
save.image("C:/Users/orosc/XXXXXXX/My DAACS/read/read_dataClean-Uni1Uni2_2.RData")
